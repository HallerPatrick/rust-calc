    ...
    pub fn lex(&self, input: &str) -> Result<Vec<Token>, String> {
        ...
        'outer: while index < input.len() {
            ...
            for &(ref regex, ref f) in &self.token_map {
                if let Some(mat) = regex.find(input_slice) {
                    let len: usize = mat.end();
...
fn create_token_map() -> Vec<(Regex, Box<Fn(&str) -> Token>)> {
    ...
    vec![
        (
            create_regex(r"^\s*\d+\.\d+"),
            Box::new(|s| Token::Float(s.trim().parse::<f64>().unwrap())),
        ),
        (
            create_regex(r"^\s*0x[0-9a-fA-F][_0-9a-fA-F]*"),
            Box::new(|s| {
        ...
        (create_regex(r"^\s*~"), Box::new(|_| Token::Tilde)),
        ...
        (
            create_regex(r"^\s*[a-zA-Z_][a-zA-Z0-9_]*\b"),
            Box::new(|s| Token::Ident(s.trim().to_string())),
        ),
    ]
}
...
mod tests {
    ...
    fn test_lex_hex() {
        ...
        assert_eq!(tokens, vec![Token::Integer(0xAB12_FFD4_CE01)]);
    }

    #[test]
    fn test_lex_bin() {
        let tokens = Lexer::new().lex("0b1001_1100_0101_0011").unwrap();
        assert_eq!(tokens, vec![Token::Integer(0b1001_1100_0101_0011)]);
    }
    ...
    fn test_lex_with_negative_float() {
        ...
        assert_eq!(tokens, vec![Token::Minus, Token::Float(123.25)]);
    }
